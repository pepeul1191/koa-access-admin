...
router.get('/system/list', [
  ...
  async (ctx, next) => {
    ...
    resp.systems = await System.find({}).select({ 
      name: 1, 
    }).skip(skip).limit(step).exec();
    // get count of systems
    resp.count = await System.countDocuments({});
    // response
  ...
  async (ctx, next) => {
    ...
    try {
      ...
      if(system_json._id == 'E'){
        ...
        var temp = await System.findOne({$or: [
        ...
        // edit system
        var temp1 = await System.findOne({
          name: system_json.name
        }).exec();
        var proceed = true;
...
router.post('/system/delete', [
  ...
  async (ctx, next) => {
    ...
    try {
      ...
      await System.findOneAndDelete(_id);
