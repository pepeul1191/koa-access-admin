...
router.get('/user/list', [
  ...
  async (ctx, next) => {
    ...
    resp.users = await User.find({}).select({
      user: 1,
      email: 2,
    }).skip(skip).limit(step).exec();
    // get count of users
    resp.count = await User.countDocuments({});
    // response
  ...
  async (ctx, next) => {
    ...
    try {
      ...
      if(user_json._id == 'E'){
        ...
        var temp = await User.findOne({$or: [
          {user: user_json.user},
